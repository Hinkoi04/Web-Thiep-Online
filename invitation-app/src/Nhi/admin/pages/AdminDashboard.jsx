...
export default function AdminDashboard() {
  ...
  const handleDelete = async (id) => {
    const token = localStorage.getItem("admin_token");
    try {
      const res = await fetch(`${API_URL}/rsvp/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      if (data.success) {
        ...
        setSuccessMsg("✓ Đã xóa lời chúc thành công.");
  ...
  const latestDate = total > 0
    ? new Date(messages[0].created_at).toLocaleDateString("vi-VN")
    : "—";
